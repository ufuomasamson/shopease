    ...
    <div class="main-content">
        <div class="page-header">
            <div class="header-content">
                <h1><i class="fas fa-edit"></i> Edit Product Review</h1>
                <p>Modify review details and update product feedback</p>
            </div>
        ...
        <div class="content-card">
            <div class="card-header">
                ...
                <p class="mb-0">Update the review details below</p>
            </div>
            
            <div class="card-body">
                <form action="{{ route('admin.reviews.update', $review) }}" method="POST" enctype="multipart/form-data" id="reviewForm">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="stars">
                                @for($i = 5; $i >= 1; $i--)
                                    <input type="radio" name="rating" id="star{{ $i }}" value="{{ $i }}" 
                                           class="star-input" required {{ $review->rating == $i ? 'checked' : '' }}>
                                ...
                                <span id="ratingText" class="text-muted">
                                    @if($review->rating)
                                        ...
                                        @switch($review->rating)
                                            @case(1) Poor - Not recommended @break
                                            @case(2) Fair - Below average @break
                                            @case(3) Good - Satisfactory @break
                                            @case(4) Very Good - Recommended @break
                                            @case(5) Excellent - Highly recommended @break
                                        @endswitch
                                    @else
                                        Select a rating
                                    @endif
                    ...
                    <div class="form-group mb-4">
                        ...
                        <textarea name="comment" id="comment" rows="5" class="form-control @error('comment') is-invalid @enderror" 
                                  placeholder="Write a detailed, positive review about the product..." required>{{ old('comment', $review->comment) }}</textarea>
                            ...
                            <div class="form-group mb-4">
                                <div class="form-check">
                                    <input type="checkbox" name="is_verified_purchase" id="is_verified_purchase" 
                                           class="form-check-input" value="1" {{ $review->is_verified_purchase ? 'checked' : '' }}>
                            ...
                            <div class="form-group mb-4">
                                <div class="form-check">
                                    <input type="checkbox" name="is_approved" id="is_approved" 
                                           class="form-check-input" value="1" {{ $review->is_approved ? 'checked' : '' }}>
                                    <label for="is_approved" class="form-check-label">
                                        <i class="fas fa-thumbs-up"></i> Approve Review
                                    </label>
                                    <div class="form-text">Review will be visible to customers</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    @if($review->images && count($review->images) > 0)
                        <div class="form-group mb-4">
                            <label class="form-label">
                                <i class="fas fa-images"></i> Current Review Images
                            </label>
                            <div class="current-images-grid">
                                @foreach($review->images as $index => $image)
                                    <div class="current-image-item">
                                        <img src="{{ Storage::url($image) }}" alt="Review image {{ $index + 1 }}" class="img-fluid">
                            ...
                            <div class="form-text">Current images will be replaced if new ones are uploaded</div>
                        </div>
                    @endif
                    ...
                    <div class="form-group mb-4">
                        <label for="images" class="form-label">
                            <i class="fas fa-images"></i> Update Review Images (Optional)
                        </label>
                        <input type="file" name="images[]" id="images" class="form-control @error('images.*') is-invalid @enderror" 
                               multiple accept="image/*">
                        <div class="form-text">Upload up to 5 new images. Max size: 2MB each. Formats: JPEG, PNG, JPG, GIF</div>
                        @error('images.*')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
...
// Show product preview on page load if product is selected
...
document.getElementById('reviewForm').addEventListener('submit', function(e) {
    ...
    submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Updating Review...';
